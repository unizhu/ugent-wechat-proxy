...
pub async fn run_server(addr: SocketAddr, broker: Arc<MessageBroker>) -> anyhow::Result<()> {
    ...
    // Bind before announcing, so a failure here is not preceded by a log line
    // claiming the server is listening.
    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .map_err(|e| anyhow::anyhow!("WeChat OA webhook could not bind {addr}: {e}"))?;
    info!("🌐 WeChat OA webhook listening on {addr}{webhook_path}");
    axum::serve(listener, build_router(state, &webhook_path)).await?;
